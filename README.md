...
Adding some RPi versions of this with real sensors(Additional .NET Core Console apps) all send to IoTHub via the service.  
  - **SimulatedSensorConsoleAppBME280**: Using **BME280** sensor, get environment data.
  - **SimulatedSensorConsoleCPUCoreTemp**: Gets the CPU Core Temperature (Note only works on Raspian, not IoT-Core). No circuitry required though. _Needs a slight rework._
